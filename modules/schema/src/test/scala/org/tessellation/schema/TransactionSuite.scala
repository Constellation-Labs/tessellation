...
object TransactionSuite extends ResourceSuite with Checkers {
  ...
  override def sharedResource: Resource[IO, Res] =
    KryoSerializer.forAsync[IO](sharedKryoRegistrar.union(schemaKryoRegistrar), List.empty, setReferences = true)

  def hashWithKryo(toHash: AnyRef): IO[Hash] =
    KryoSerializer
      .forAsync[IO](sharedKryoRegistrar.union(schemaKryoRegistrar), List.empty, setReferences = true)
      .use { kryo =>
        implicit val k = kryo
        toHash.hashF
      }
