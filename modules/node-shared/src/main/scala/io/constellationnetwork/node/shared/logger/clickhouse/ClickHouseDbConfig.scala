...
object ClickHouseDbConfig {
  ...
  private def stripProtocol(host: String): String =
    host
      .stripPrefix("https://")
      .stripPrefix("http://")
      .stripSuffix("/")
  ...
  private def validateHost(host: String): Either[ConfigValidationError, String] = {
    val cleaned = stripProtocol(host)
    if (hostnamePattern.matches(cleaned) || ipv4Pattern.matches(cleaned) || ipv6Pattern.matches(cleaned)) Right(host)
