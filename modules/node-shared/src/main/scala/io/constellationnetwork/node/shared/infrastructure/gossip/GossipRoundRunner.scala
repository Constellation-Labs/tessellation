  ...
  def make[F[_]: Async: Random: Metrics](
    ...
    for {
      ...
      new GossipRoundRunner[F] {
        private val logger = Slf4jLogger.getLoggerFromClass[F](GossipRoundRunner.getClass)
        ...
        private def evalRound(peer: Peer): F[Unit] =
          logger.trace(s"Starting gossip round with {peer=${peer.show}}") >>
                  ...
                  (for {
                    ...
                    _ <- result match {
                      case Right((duration, _)) =>
                        logger.trace(s"Gossip round succeeded {peer=${peer.show}, duration=${duration.toMillis}ms}") >>
        ...
        private def selectPeers: F[Unit] =
          for {
            ...
            _ <- logger.trace(
              s"Gossip round: " +
                s"allPeers=${allPeers.size} " +
                s"selectedPeers=${selectedPeers.size} " +
                s"availablePeers=${availablePeers.size} " +
                s"drawnPeers=${drawnPeers.size}"
            )
