  ...
  def make[F[_]: Async, Event, Key, Artifact, Context, Status, Outcome, Kind](
    ...
    Async[F].pure {
      new ConsensusManager[F, Event, Key, Artifact, Context, Status, Outcome, Kind] {
        ...
        def registerForConsensus(observationKey: Key): F[Unit] =
          storage.trySetObservationKey(observationKey).flatMap {
            case true  => nodeStorage.tryModifyState(NodeState.WaitingForObserving, NodeState.Observing)
            case false => new Throwable("Registration failed: already registered at different key").raiseError[F, Unit]
          }
