...
object RecoveryGlobalSnapshotSync {
  ...
  /** A reset is distinguishable from an ordinary first declaration by the authoritative singleton signer set. A newly admitted signer in
    * a multi-member committee may legitimately publish a MinValue-parent declaration and must stay on the ordinary validation path.
    */
  def hasResetShape(
    signer: PeerId,
  ...
  def validateReset(
    ...
    Either
      ...
      .flatMap(_ =>
        Either.cond(
          selectedTarget.exists(target => target >= oldest && target <= context.currentGlobalParent && context.recentGlobalSnapshots.contains(target)),
          (),
