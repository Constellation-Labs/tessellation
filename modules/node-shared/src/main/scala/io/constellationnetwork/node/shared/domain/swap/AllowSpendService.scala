        ...
        .flatMap {
          case Valid(_) =>
            lastSnapshotStorage.getCombinedStream.map {
              case Some((s, si)) => (s.ordinal, s.epochProgress, si.balances.getOrElse(allowSpend.source, Balance.empty))
              case None          => (SnapshotOrdinal.MinValue, EpochProgress.MinValue, Balance.empty)
            }.changes.switchMap {
              case (latestOrdinal, latestEpochProgress, balance) =>
                Stream.eval(allowSpendStorage.tryPut(allowSpend, latestOrdinal, latestEpochProgress, balance))
