...
/** Proof that a majority of facilitators agreed a round timed out.
  *
  * Modeled after HotStuff's TimeoutCertificate: formed when a majority of facilitators independently report the same
  * stall. The `agreedMissingPeers` field (intersection of all signers' missing peer lists) enables deterministic
  * eviction — all nodes compute the same removal set from the same TC data.
  */
...
/** Event-driven aggregator for StallReport messages, modeled after Flow's TimeoutAggregator.
  *
  * Receives StallReports from facilitators (local detection or remote gossip) and forms a [[TimeoutCertificate]] when a
  * majority agree the round is stalled. The TC fires a callback to [[ViewChangeManager]] which performs deterministic
  * eviction and leader rotation.
  *
  * ==Architecture==
  *
  * Unlike the previous polling-based EvictionVoteTracker, this component is '''event-driven''': each incoming
  * StallReport is processed immediately via [[addStallReport]], and the TC callback fires the instant the
  * quorum-completing report arrives. No polling loop, no timing races with gossip delivery.
  *
  * ==Flow==
  * {{{
...
trait TimeoutAggregator[F[_]] {
  ...
  /** Process a StallReport from a facilitator (local or remote).
    * If this report completes a majority, the onTcFormed callback fires immediately.
    */
  ...
  def make[F[_]: Async](
    ...
    for {
            ...
            for {
              ...
              _ <- (
                for {
                  ...
                  tc = buildTC(roundId, leaderEpoch, collected)
                  _ <- firedRef.set(true)
                  _ <- onTcFormed(tc)
                } yield ()
      ...
      private def buildTC(
        ...
        val agreedMissing = allReportedMissing.filter { peer =>
