...
/** Finite State Machine that routes consensus commands to appropriate handlers.
  ...
  */
class ConsensusFSM[F[_]: Async: Metrics: HasherSelector: Random, Event, Key, Artifact, Ctx, Status, Outcome, Kind](
  ...
  private def startRound(trigger: Option[ConsensusTrigger]): F[Unit] =
    isRunning.get.flatMap {
      case true => log.debug(s"Ignoring StartRound($trigger) — round already running")
      case false =>
        log.info(s"Starting consensus round with trigger=$trigger") >>
          isRunning.set(true) >>
