...
/** Structural policy for v35 Global L0 signing-lease changes.
  *
  * A certified proposal may expand the committee using the existing admission lane, but it may
  * never contract it. An eviction is valid only as one half of an equal-sized atomic replacement.
  * The complete admitted/evicted sets are already covered by the v35 ProposalValue QC, so this
  * helper deliberately introduces no new message field or hash domain.
  */
object CertifiedMembershipTransition {
  ...
  /** Validate the list-shaped certificate surface before reducing it to the set-shaped
    * [[CertifiedConsensus.ProposalValue]]. Proposal validation already checks each ACS/ECS
    * independently; keeping the duplicate-target rule here as well makes the atomic transition
    * fail closed if that validation pipeline is ever reordered.
    */
  ...
  /** Validate and apply while preserving the inherited committee order and appending new
    * admissions in stable PeerId order, matching existing admission semantics.
    */
  def applyTo(
  ...
  /** Local Core prepare-vote policy for a structurally valid certified membership value.
    *
    * Proof subsets are intentionally local and therefore can only make this voter abstain from
    * an admission-only expansion. They never alter validation or the derived next committee. An
    * equal-sized replacement keeps Q(N) unchanged and is independent of the local proof subset.
    */
