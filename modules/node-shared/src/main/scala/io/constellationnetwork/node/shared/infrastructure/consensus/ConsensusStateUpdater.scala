...
object ConsensusStateUpdater {
  ...
  def make[F[
    ...
    new ConsensusStateUpdater[F, Key, Artifact, Context, Status, Outcome, Kind] {
      ...
      def tryUpdateConsensus(key: Key, resources: ConsensusResources[Artifact, Kind]): F[StateUpdateResult] =
        logger.debug(s"tryUpdateConsensus called for key=$key") >>
      ...
      private def tryUpdateExistingConsensus(
        ...
        logger.debug(s"tryUpdateExistingConsensus called for key=$key") >>
          consensusStorage
            .condModifyState(key)(toUpdateStateFn(fn))
            .flatTap(result => logger.debug(s"condModifyState returned ${result.isDefined} for key=$key"))
            .flatMap(evalEffect)
            .flatTap(result => logger.debug(s"evalEffect returned ${result.isDefined} for key=$key"))
