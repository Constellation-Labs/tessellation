            ...
            for {
              ...
              result <- applyCache(state, currencyIncrementalSnapshot.ordinal) >>= {
                case (latestState, latestOrdinal) =>
                  dataApplication.setCalculatedState(latestOrdinal, latestState.calculated).flatMap { wasSet =>
                    logger.warn(s"setCalculatedState returned false for ordinal=${latestOrdinal.show}").unlessA(wasSet)
                  } >>
