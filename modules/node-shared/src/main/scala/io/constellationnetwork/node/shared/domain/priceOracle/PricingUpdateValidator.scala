...
object PricingUpdateValidator {
  def make[F[_]: Async](allowedCurrencyIds: Option[List[Address]], minEpochsBetweenUpdates: NonNegLong): PricingUpdateValidator[F] =
    new PricingUpdateValidator[F] {
      ...
      override def validate(
        ...
        for {
          currencyIdR <- validateCurrencyId(pricingUpdate, currencyId).pure[F]
          epochProgressR = validateEpochProgress(pricingUpdate, lastContext, epochProgress)
        } yield currencyIdR.productR(epochProgressR)
      ...
      private def validateCurrencyId(pricingUpdate: PricingUpdate, address: Address): PricingUpdateValidationErrorOr[PricingUpdate] =
        if (allowedCurrencyIds.isEmpty || allowedCurrencyIds.exists(_.contains(address))) {
          pricingUpdate.validNec[PricingUpdateValidationError]
        } else {
          UnauthorizedCurrency(address).invalidNec
        }
  ...
  case class UnauthorizedCurrency(currencyId: Address) extends PricingUpdateValidationError
