...
object ConsensusManager {
  ...
  def make[F[_]: Async: Metrics: Random, Event, Key: Show: Order: Next, Artifact: Eq, Context: Eq, Status: Eq, Outcome, Kind](
    ...
    ConsensusMetricsLogger.make[F].flatMap { metricsLogger =>
                  ...
                  .flatMap { resources =>
                    Clock[F].monotonic.flatMap { endTime =>
                      logger.debug(s"Trying to facilitate consensus {key=${nextKey.show}, trigger=${trigger.show}}") >>
                        metricsLogger.logTriggerProcessing(
                          trigger.map(_.getClass.getSimpleName).getOrElse("None"),
                          endTime - startTime
                        )
                    } >>
              ...
              consensusStateAdvancer.getConsensusOutcome(newState) match {
                case Some((previousKey, newOutcome)) =>
                  Clock[F].monotonic.flatMap { finishedAt =>
                    val consensusDuration = finishedAt - newState.createdAt
                    Metrics[F].recordTime("dag_consensus_duration", consensusDuration) >>
                      Metrics[F].recordTime(
                        "dag_consensus_duration_by_trigger",
                        consensusDuration,
                        Seq(("trigger_type", _trigger.get(newOutcome).getClass.getSimpleName))
                      ) >>
                      metricsLogger.logLastSnapshotTime(_key.get(newOutcome).show, finishedAt)
                  } >>
                    consensusStorage
        ...
        private def afterConsensusFinish(majorityTrigger: ConsensusTrigger): F[Unit] =
          Clock[F].monotonic.flatMap { startTime =>
            (majorityTrigger match {
              ...
              metricsLogger.logTriggerProcessing(
                s"${majorityTrigger.getClass.getSimpleName}Finish",
                endTime - startTime
              )
            }
          }
