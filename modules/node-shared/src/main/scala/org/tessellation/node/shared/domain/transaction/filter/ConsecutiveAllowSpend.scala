...
import org.tessellation.schema.swap.{AllowSpend, AllowSpendReference}
import org.tessellation.security.signature.Signed
import org.tessellation.security.{Hashed, Hasher}

object ConsecutiveAllowSpend {
  def signedTxOrder: Order[Signed[AllowSpend]] =
    Order.whenEqual(Order.by(-_.fee.value.value), Order[Signed[AllowSpend]])

  def hashedTxOrder: Order[Hashed[AllowSpend]] = signedTxOrder.contramap(_.signed)
  ...
  def take(
    ...
    takeGeneric[Id, AllowSpendReference, Hashed[AllowSpend]](
      ...
      txs,
      lastAcceptedTxRef
    )(Applicative[Id], Order[AllowSpendReference], hashedTxOrder)
