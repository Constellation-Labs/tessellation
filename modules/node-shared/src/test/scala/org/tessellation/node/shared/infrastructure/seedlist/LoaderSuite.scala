...
object LoaderSuite extends SimpleIOSuite with Checkers {
  ...
  test("load 4-fields-format seedlist csv") {
  ...
  test("load 5-field-format seedlist csv") {
  ...
  test("load invalid 5-fields-format seedlist csv") {
    val testFileLocation = getClass.getResource("/seedlists/seedlist.5fields.invalid.sample").getPath
    val inFile = Path(testFileLocation)
  ...
  test("load invalid-format seedlist csv") {
    ...
    for {
      loader <- Loader.make[IO].load(SeedListPath(inFile)).attempt
      maybeErrorMessage = loader.left.map(_.getMessage)
    } yield expect.eql(maybeErrorMessage, Left("Rows must have 1, 4, or 5 fields, but found 3"))
  }
