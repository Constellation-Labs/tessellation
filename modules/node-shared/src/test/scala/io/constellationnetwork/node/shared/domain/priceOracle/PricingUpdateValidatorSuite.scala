...
import io.constellationnetwork.node.shared.domain.priceOracle.PricingUpdateValidator.{TooFrequentUpdate, UnauthorizedCurrency}
...
object PricingUpdateValidatorSuite extends MutableIOSuite {
  ...
  test("currency whitelists") { res =>
    val (kp1, kp2) = res

    val address1 = kp1.getPublic.toAddress
    val address2 = kp2.getPublic.toAddress

    val allowedCurrencyIds = List(address1).some

    val validator = PricingUpdateValidator.make[IO](allowedCurrencyIds, NonNegLong(0))
    ...
    } yield expect.all(accepted == List(update1), rejected == List((update2, List(UnauthorizedCurrency(address2)))))
  }
