...
object ControllerEvidenceDerivationSuite extends SimpleIOSuite {
  ...
  // ===========================================================================
  // recentParticipants / idleWindows / tenureWindows (reward-rotation inputs)
  // ===========================================================================

  pureTest("recentParticipants is the union of completedSigners over the trailing DemotionConsecutiveMisses entries") {
    // window of 5; the trailing 3 (ordinals 3,4,5) decide. a signs all; b only at ord 1 (rotated
    // out of the trailing 3); c at ord 4; d at ord 5.
    val evidence = window(
      1L -> entry(Set(a, b), Set(a, b)),
      2L -> entry(Set(a), Set(a)),
      3L -> entry(Set(a), Set(a)),
      4L -> entry(Set(a, c), Set(a, c)),
      5L -> entry(Set(a, d), Set(a, d))
    )
    expect.same(Set(a, c, d), ControllerEvidenceDerivation.recentParticipants(evidence))
  }

  pureTest("recentParticipants is empty on an empty window") {
  ...
  pureTest("idleWindows counts trailing entries since the peer last signed") {
    ...
    // a signs the latest entry -> idle 0. b last signed at ord 1 -> 3 trailing non-signing entries.
    expect.same(0, ControllerEvidenceDerivation.idleWindows(evidence, a)) &&
    expect.same(3, ControllerEvidenceDerivation.idleWindows(evidence, b))
  }

  pureTest("idleWindows equals the full window length for a peer that never signed, and is 0 on empty") {
    val evidence = window(
      1L -> entry(Set(a), Set(a)),
      2L -> entry(Set(a), Set(a))
    )
    // e never appears -> idle equals the window size (2).
    expect.same(2, ControllerEvidenceDerivation.idleWindows(evidence, e)) &&
    expect.same(0, ControllerEvidenceDerivation.idleWindows(SortedMap.empty, e))
  }

  pureTest("tenureWindows counts consecutive trailing entries the peer signed") {
    val evidence = window(
      1L -> entry(Set(a, b), Set(a)),
      2L -> entry(Set(a, b), Set(a, b)),
      3L -> entry(Set(a, b), Set(a, b))
    )
    // a signs all 3 trailing -> tenure 3. b signs the latest 2 consecutively -> tenure 2.
    expect.same(3, ControllerEvidenceDerivation.tenureWindows(evidence, a)) &&
    expect.same(2, ControllerEvidenceDerivation.tenureWindows(evidence, b))
  }

  pureTest("tenureWindows is 0 when the peer is absent from the latest entry") {
    val evidence = window(
      1L -> entry(Set(a, b), Set(a, b)),
      2L -> entry(Set(a, b), Set(a))
    )
    // b signed at ord 1 but not the latest -> tenure 0 (streak must include the most recent entry).
    expect.same(0, ControllerEvidenceDerivation.tenureWindows(evidence, b)) &&
    expect.same(0, ControllerEvidenceDerivation.tenureWindows(SortedMap.empty, a))
  }

  pureTest("recentParticipants / idleWindows / tenureWindows are deterministic for the same window") {
    val evidence = window(
      1L -> entry(Set(a, b, c), Set(a, b)),
      2L -> entry(Set(a, b), Set(a)),
    ...
    expect.same(ControllerEvidenceDerivation.idleWindows(evidence, b), ControllerEvidenceDerivation.idleWindows(evidence, b)) &&
    expect.same(ControllerEvidenceDerivation.tenureWindows(evidence, a), ControllerEvidenceDerivation.tenureWindows(evidence, a))
  }
