...
object SnapshotLocalFileSystemStorageSuite extends MutableIOSuite with Checkers {
  ...
  test("cleanupAboveOrdinal skips decoding orphans older than the anchor but still removes recent ones") { res =>
    implicit val (_, kryo, j, h, sp, gsps) = res
    implicit val selector: HasherSelector[IO] = HasherSelector.forSyncAlwaysCurrent(h)

    // The orphan pass can only avoid decoding a candidate by ruling it out cheaply. `nlink` does
    // that on a healthy store, but a store copied without `rsync -H` has intact content and
    // nlink==1 everywhere, so the fast path is gone and ~58% of entries get decoded -- 89 minutes
    // on the real testnet store, to delete nothing. mtime restores a cheap rule-out: a snapshot
    // ABOVE the anchor cannot have been written before the anchor itself.
    //
    // Both directions matter. Skipping too eagerly would silently keep a torn write servable by
    // hash, which is the whole reason this pass exists.
    ...
    File.temporaryDirectory() { tmpDir =>
      for {
        ...
        // The anchor keeps its ordinal link, so anchorWriteTime has a reference to compare against.
        anchorKey <- KeyPairGenerator.makeKeyPair[IO]
        anchorSigned <- Signed.forAsyncHasher[IO, GlobalIncrementalSnapshot](
          base.value.copy(ordinal = anchor),
          anchorKey
        )
        _ <- storage.write(anchorSigned)
        ...
        staleOrphan <- mkOrphan(40001L)
        recentOrphan <- mkOrphan(40002L)

        // Backdate the stale one well past the margin, as a migrated store's ancient bodies are.
        stalePath <- storage.getPath(staleOrphan)
        _ <- IO.blocking {
          java.nio.file.Files.setLastModifiedTime(
            stalePath.path,
            java.nio.file.attribute.FileTime.fromMillis(System.currentTimeMillis() - 30L * 24 * 3600 * 1000)
          )
        }

        _ <- storage.cleanupAboveOrdinal(anchor, (hash, ordinal) => storage.delete(hash) >> storage.delete(ordinal))

        keptStale <- storage.read(staleOrphan)
        removedRecent <- storage.read(recentOrphan)
      } yield expect.all(keptStale.nonEmpty, removedRecent.isEmpty)
    }
  }
