...
import io.constellationnetwork.node.shared.infrastructure.gossip.p2p.{GossipClient, StockGossipClient}
...
/** Stock reproduction, with real runner/client and a controlled response-body fixture. Session verification is a stub: this is not
  * authentication or native-network qualification.
  */
object StockGossipSlotStarvationSuite extends SimpleIOSuite {
      ...
      for {
        ...
        transport = Client[IO] { _ =>
          Resource.make(
            IO.pure(
              Response[IO]()
                .putHeaders(`X-Id`(peer(1).id))
                .withBodyStream(Stream.never[IO])
            )
          )(_ => releases.update(_ + 1))
        }
        client = StockGossipClient.make(transport, session, GossipTimeoutsConfig(10.seconds, 5.seconds))
        finished <- Ref.of[IO, Boolean](false)
        fiber <- client
          .queryPeerRumors(PeerRumorInquiryRequest(Map.empty))
          .run(peer(1))
          .compile
          .drain
          .guarantee(finished.set(true))
          .start
  ...
  List(false, true).foreach { corrected =>
    test(s"eight stalled gossip bodies: corrected=$corrected; healthy queued peer progresses only with a response deadline") {
      TestControl.executeEmbed {
          ...
          Random.scalaUtilRandomSeedInt[IO](0).flatMap { implicit random =>
            for {
              ...
              transport = Client[IO] { req =>
                ...
                Resource.make(started.update(_ + n).as(Response[IO]().putHeaders(`X-Id`(peer(n).id)).withBodyStream(body)))(_ =>
                  released.update(_ + 1)
                )
              }
              client =
                if (corrected) GossipClient.make(transport, session, GossipTimeoutsConfig(10.seconds, 5.seconds))
                else StockGossipClient.make(transport, session, GossipTimeoutsConfig(10.seconds, 5.seconds))
              ...
              runner <- GossipRoundRunner.make[IO](
                ...
                "peer",
                GossipRoundConfig(1, 200.millis, 8)
              )
              _ <- runner.runForever
              _ <- IO.sleep(3.seconds)
              initial <- started.get
              _ <- cluster.addPeer(healthy)
              _ <- IO.sleep(if (corrected) 9.seconds else 180.seconds)
              requests <- started.get
              completed <- successes.get
              errors <- healthchecks.get
              freed <- released.get
            } yield
              expect.same(initial, (1 to 8).toSet) &&
                (if (corrected) expect(requests.contains(9)) && expect(completed > 0) && expect(errors >= 8) && expect(freed >= 8)
                 else expect.same(requests, initial) && expect.same(completed, 0) && expect.same(errors, 0) && expect.same(freed, 0))
          }
        }
      }
    }
