...
object CertifiedVoteLockPersistenceSuite extends MutableIOSuite {
  ...
  private def certifiedQc(implicit
    hasher: Hasher[IO],
    provider: SecurityProvider[IO]
  ): IO[(CertifiedProposalQC, Set[PeerId], java.security.KeyPair)] =
    for {
    ...
    } yield (qc, ids.toSet, pairs.head)

  test("atomic journal round-trips across a fresh storage instance") { case (base, serializer, _, _) =>
  ...
  test("missing is None but corrupt or truncated journal bytes fail closed") { case (base, serializer, _, _) =>
    implicit val json: JsonSerializer[IO] = serializer
  ...
  test("an identical clean lock is not forced to disk again on prepare re-entry") { case (_, _, _, _) =>
    for {
  ...
  test("a journal write failure prevents the caller from reaching vote emission") { case (_, _, hasher, provider) =>
  ...
  test("a QC journal failure prevents vote transport and commit progression") { case (_, _, hasher, provider) =>
    implicit val h: Hasher[IO] = hasher
    implicit val sp: SecurityProvider[IO] = provider
  ...
  test("cancellation during persistence cannot expose a non-durable lock or emit a vote") { case (_, _, hasher, provider) =>
    implicit val h: Hasher[IO] = hasher
    implicit val sp: SecurityProvider[IO] = provider
  ...
  test("dirty reads retry persistence and fail closed until the stricter lock is durable") { case (_, _, _, _) =>
    for {
      attempts <- Ref.of[IO, Int](0)
      persisted <- Ref.of[IO, Option[CertifiedVoteLock]](None)
      persistence = new CertifiedVoteLockPersistence[IO, SnapshotOrdinal] {
  ...
  test("verified carried QC persists before progress even when local vote emission is disabled") { case (_, _, hasher, provider) =>
    implicit val h: Hasher[IO] = hasher
    implicit val sp: SecurityProvider[IO] = provider

    for {
      built <- certifiedQc
      (qc, committee, corePair) = built
      persisted <- Ref.of[IO, Option[CertifiedVoteLock]](None)
      writes <- Ref.of[IO, Int](0)
      emitted <- Ref.of[IO, Boolean](false)
      persistence = new CertifiedVoteLockPersistence[IO, SnapshotOrdinal] {
