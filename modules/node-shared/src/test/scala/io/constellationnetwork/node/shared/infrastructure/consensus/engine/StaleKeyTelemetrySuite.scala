...
/** D1 stale-key WARN rate limiting: one initial WARN per key, at most one reminder per interval, reset only on accepted consensus
  * progress (`reset`, wired to `resetOnSuccessfulRound`) and never by the abandonment path's own `RoundCompleted`, bounded retention.
  * The clock is injected so no test sleeps.
  */
  ...
  test("first capture at a key emits the initial WARN; a same-key retry inside the interval is suppressed") {
    fixture().flatMap {
      case (clock, telemetry) =>
        ...
        } yield expect(first.exists(_.kind == "initial"), s"first capture must be the initial WARN, got $first")
  ...
  test("at most one reminder per minute, with residence measured from the first local observation") {
    fixture().flatMap {
      case (clock, telemetry) =>
        ...
        } yield expect(early.isEmpty, "59s after the initial WARN no reminder is due")
  ...
  test("abandonment (RoundCompleted without accepted progress) does not reset the budget; accepted progress does") {
    fixture().flatMap {
      case (clock, telemetry) =>
        ...
        } yield expect(afterAbandon.isEmpty, "a same-key retry after abandonment stays rate limited")
          .and(expect(afterProgress.exists(_.kind == "initial"), s"after accepted progress the key gets a fresh initial WARN, got $afterProgress"))
    }
  }
  ...
  test("keys below the installed parent are evicted on capture and the map is capped") {
    fixture(maxEntries = 2).flatMap {
      case (clock, telemetry) =>
        ...
        } yield expect.same(Set(101L, 102L), afterCap)
    ...
    fixture().flatMap {
      case (clock, telemetry) =>
        for {
          ...
          _ <- telemetry.observe(100L, 2)
          reminder <- telemetry.capture(100L, 99L)
        } yield expect(none.exists(_.lastExternalFacilityAgo.isEmpty), "no external Facility yet reports unknown")
          .and(
            expect(
