  ...
  test("a partial timed-out daemon round advances the cursor in the next request") {
    TestControl.executeEmbed {
      ...
      for {
        ...
        first <- GossipDaemon.runPeerRound(storage, client, peer)(storage.addPeerRumorIfConsecutive(_).void).attempt
        second <- GossipDaemon.runPeerRound(storage, client, peer)(storage.addPeerRumorIfConsecutive(_).void).attempt
