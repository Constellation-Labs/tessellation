  ...
  private def writeNewAtomically[F[_]: Async](target: JPath, bytes: Array[Byte]): F[Unit] =
    Async[F].blocking {
      ...
      try {
        ...
        // Publish with an exclusive same-filesystem hard link. `createLink` is atomic and fails if
        // the target already exists, unlike `ATOMIC_MOVE`, whose replacement behavior is explicitly
        // implementation-specific when the target races into existence.
        JFiles.createLink(absolute, temporary)
        JFiles.delete(temporary)

        // Linux supports fsync on a directory and that makes the rename durable. Some filesystems do not; the plan file itself remains
        // fully written and atomically named in that case.
        try {
