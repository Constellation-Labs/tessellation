...
trait SentStateChannelBinaryTrackingService[F[_]] {
  def setPending(binary: Signed[StateChannelSnapshotBinary])(implicit hasher: Hasher[F]): F[Unit]
  def getRetriable: F[List[Signed[StateChannelSnapshotBinary]]]
  def updateByGlobalSnapshot(globalSnapshot: GlobalIncrementalSnapshot)(implicit hasher: Hasher[F]): F[Unit]
}
...
object SentStateChannelBinaryTrackingService {
  ...
  def make[F[_]: Async](
    ...
    new SentStateChannelBinaryTrackingService[F] {
      ...
      private def updateByGlobalSnapshotBinaries(binaries: List[Signed[StateChannelSnapshotBinary]])(implicit hasher: Hasher[F]): F[Unit] = {
