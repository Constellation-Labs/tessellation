...
import io.constellationnetwork.currency.l0.snapshot.synchronous.message.{GetConsensusOutcomeRequest, RegistrationResponse}
  ...
  private val p2p: HttpRoutes[F] = HttpRoutes.of[F] {
    ...
    case req @ POST -> Root / "specific" / "outcome" => // POST used instead of GET because `Key` can't be used be in path
      for {
        ...
        result <- storage.getConsensusOutcome(outcomeRequest.key).flatMap {
          case Some(value) => Ok(value.some)
          case None =>
            storage.getLastKey.flatMap {
              case Some(lastKey) if lastKey > outcomeRequest.key => Conflict()
