...
trait ConsensusStateAdvancer[F[_], Key, Artifact, Context, Status, Outcome, Kind] {
  ...
  protected def maybeGetAllDeclarations[A](
    ...
    for {
      ...
      isStale = elapsed > config.declarationTimeout
      result <-
        if (isStale) {
          logger.warn(
            s"The process is stale when getting all declarations. Elapsed: ${elapsed.toSeconds}s, Timeout: ${config.declarationTimeout.toSeconds}s"
          ) >> processStale.pure
        } else {
