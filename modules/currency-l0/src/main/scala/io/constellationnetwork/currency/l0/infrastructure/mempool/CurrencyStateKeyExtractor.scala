...
/** Extracts CurrencyStateKey from CurrencySnapshotEvent.
  *
  * Keys identify which state partitions an event modifies, enabling conflict detection for parallel processing in the mempool.
  *
  * Since each currency-l0 instance handles only one metagraph, no metagraph namespace scoping is needed.
  */
