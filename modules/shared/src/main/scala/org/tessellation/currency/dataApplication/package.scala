...
import org.tessellation.currency.dataApplication.DataTransaction.{DataTransactions, collectTransactions}
import org.tessellation.currency.dataApplication.Errors.Noop
import org.tessellation.currency.dataApplication.FeeTransaction.{feeTransactionRequestDecoder, serialize}
...
object FeeTransaction {
  ...
  def feeTransactionRequestDecoder[F[_]: Async, D <: DataUpdate](
    ...
    implicit val signedDataUpdateDecoder: Decoder[Signed[D]] = Signed.decoder[D]
    implicit val signedFeeTransactionDecoder: Decoder[Signed[FeeTransaction]] = Signed.decoder[FeeTransaction](feeTransactionDecoder)
...
object Errors {
  ...
  case class NotEnoughFee() extends DataApplicationValidationError {
    val message = "Not enough fees"
  }
  case class SourceWalletNotEnoughBalance() extends DataApplicationValidationError {
    val message = "Source wallet not enough balance"
  }
  case class MissingDataUpdateOfFeeTransaction() extends DataApplicationValidationError {
    val message = "Could not find data update for provided fee transaction"
  }
  case class MissingFeeTransaction() extends DataApplicationValidationError {
    val message = "Missing fee transaction"
  }
  case class SourceWalletNotSignTheTransaction() extends DataApplicationValidationError {
...
trait BaseDataApplicationL1Service[F[_]] extends BaseDataApplicationService[F] with BaseDataApplicationL1ContextualOps[F] {
  def requestDecoder(implicit f: Async[F]): EntityDecoder[F, DataTransactions]
}
...
object BaseDataApplicationL1Service {
  def apply[F[+_], D <: DataUpdate, DON <: DataOnChainState, DOF <: DataCalculatedState](
    ...
    new BaseDataApplicationL1Service[F] {
      ...
      override def requestDecoder(implicit f: Async[F]): EntityDecoder[F, DataTransactions] =
        service.requestDecoder
...
object dataApplication {
  ...
  case class DataApplicationBlock(
    roundId: RoundId,
    updates: NonEmptyList[DataTransactions],
    updatesHashes: NonEmptyList[NonEmptyList[Hash]]
  ) extends Encodable[NonEmptyList[NonEmptyList[Hash]]] {
    override def toEncode: NonEmptyList[NonEmptyList[Hash]] = updatesHashes
  ...
  object DataApplicationBlock {
    ...
    implicit def eqv: Eq[DataApplicationBlock] =
      Eq.and[DataApplicationBlock](
        ...
        Eq[NonEmptyList[NonEmptyList[Hash]]].contramap(_.updatesHashes)
      )
  }
