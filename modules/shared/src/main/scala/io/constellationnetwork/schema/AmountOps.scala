...
object AmountOps {
  ...
  implicit class BigDecimalOps(val value: BigDecimal) extends AnyVal {
     ...
     *
     * @param F The monad with error handling
     * @return The value as an Amount wrapped in F
     */
    def toAmount[F[_]: MonadThrow]: F[Amount] =
      if (value <= 0) Amount.empty.pure[F]
      else MonadThrow[F].fromEither(
        NonNegLong.from(value.setScale(0, RoundingMode.HALF_UP).longValue)
          .bimap(
     ...
     *
     * @param F The monad with error handling
     * @return The value as a TransactionAmount wrapped in F
     */
    def toTransactionAmount[F[_]: MonadThrow]: F[TransactionAmount] =
      if (value <= 0) MonadThrow[F].raiseError(new IllegalArgumentException("Transaction amount must be positive"))
        ...
        PosLong.from(value.setScale(0, RoundingMode.HALF_UP).longValue)
          .bimap(
