...
object priceOracle {
  ...
  @derive(decoder, encoder, keyEncoder, show, order, ordering)
  case object BTC extends CryptoToken
  @derive(decoder, encoder, keyEncoder, show, order, ordering)
  case object ETH extends CryptoToken
  ...
  object TokenPair {
    ...
    val BTC_USD = TokenPair(BTC, USD)
    val ETH_USD = TokenPair(ETH, USD)
  }
  ...
  object PricingUpdateReference {
    ...
    def of[F[_]: Async: Hasher](signedTransaction: Signed[PricingUpdate]): F[PricingUpdateReference] =
      signedTransaction.value.hash.map(PricingUpdateReference(signedTransaction.ordinal, _))
  }
