...
import io.constellationnetwork.json.StreamingCollectionEncoder._
...
object JsonBrotliBinarySerializer {
  ...
  private def streamPrintAndCompress[A: Encoder](content: A, printer: Printer, params: Parameters): Array[Byte] = {
    ...
    try {
      ...
      encoder match {
        ...
        case _ =>
          val wrapped = StreamingCollectionEncoder.wrapEncoder(encoder)
          wrapped.streamEncode(content, printer, appendable)
      }
    } finally
      brotli.close()
