    ...
    new MerklePatriciaRangeVerifier[F] {
      ...
      def confirmRange(proof: MerklePatriciaRangeProof): F[Either[MerklePatriciaVerificationError, Unit]] = {
        ...
        def verifyPathsInRange: F[Either[MerklePatriciaVerificationError, Unit]] = {
          ...
          allPathsInRange.pure[F].ifM(
            ifTrue = ().asRight[MerklePatriciaVerificationError].pure[F],
            ifFalse =
              (InvalidPath("Some paths in range proof are outside the specified range"): MerklePatriciaVerificationError)
              .asLeft[Unit]
              .pure[F]
          )
        }
        ...
        def verifyPathsOrdered: F[Either[MerklePatriciaVerificationError, Unit]] = {
          ...
          (paths == sortedPaths).pure[F].ifM(
            ifTrue = ().asRight[MerklePatriciaVerificationError].pure[F],
            ifFalse =
              (InvalidWitness("Paths in range proof are not in sorted order"): MerklePatriciaVerificationError)
              .asLeft[Unit]
              .pure[F]
          )
        }
            ...
            case Some(boundaries) =>
              ...
              for {
                leftResult <- boundaries.leftBoundary match {
                  case Some(leftProof) =>
                    (hexOrdering.compare(leftProof.path, proof.startPath) < 0).pure[F].ifM(
                      ifTrue = singleVerifier.confirm(leftProof),
                      ifFalse = (InvalidPath(s"Left boundary ${leftProof.path.value} must be < startPath ${proof.startPath.value}"): MerklePatriciaVerificationError)
                        .asLeft[Unit]
                        .pure[F]
                    )
                  case None =>
                ...
                rightResult <- boundaries.rightBoundary match {
                  case Some(rightProof) =>
                    (hexOrdering.compare(rightProof.path, proof.endPath) > 0).pure[F].ifM(
                      ifTrue = singleVerifier.confirm(rightProof),
                      ifFalse =
                        (InvalidPath(s"Right boundary ${rightProof.path.value} must be > endPath ${proof.endPath.value}"): MerklePatriciaVerificationError)
                        .asLeft[Unit]
                        .pure[F]
                    )
                  case None =>
        ...
        (hexOrdering.compare(proof.startPath, proof.endPath) > 0).pure[F].ifM(
          ifTrue = (InvalidPath(s"Invalid range: startPath ${proof.startPath.value} > endPath ${proof.endPath.value}"): MerklePatriciaVerificationError)
            .asLeft[Unit]
            .pure[F],
          ifFalse = {
            for {
