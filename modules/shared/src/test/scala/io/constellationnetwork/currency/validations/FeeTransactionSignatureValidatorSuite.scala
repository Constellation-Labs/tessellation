...
object FeeTransactionSignatureValidatorSuite extends MutableIOSuite {
  ...
  pureTest("activates inclusively at the configured global snapshot ordinal") {
    val activationOrdinal = SnapshotOrdinal.unsafeApply(100L)

    expect.all(
      !isEnabled(SnapshotOrdinal.unsafeApply(99L), activationOrdinal),
      isEnabled(activationOrdinal, activationOrdinal),
      isEnabled(SnapshotOrdinal.unsafeApply(101L), activationOrdinal)
    )
  }
