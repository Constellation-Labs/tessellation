  ...
  def make[F[_]: Async: Parallel: SecurityProvider: Metrics: HasherSelector](
    ...
    private def toProposalsPhase(
      ...
      for {
        ...
        // Debug logging for hash union and sync
        hashSetsWithPeers = facilities.map { case (peerId, f) => (peerId.show.take(8), f.eventHashes.size) }
        _ <- logger.debug(
    ...
    /** Sync missing events from peers who have them.
      *
      * Groups missing hashes by peer and fetches in parallel using IWANT requests.
      */
    private def syncMissingEvents(
      missingHashes: Set[Hash],
      hashToPeer: Map[Hash, PeerId]
    ): F[Unit] = {
      // Group missing hashes by which peer has them
