  ...
  def make[F[_]: Async: Parallel: SecurityProvider: Metrics: HasherSelector](
    ...
    private def resolveLeaderProposal(
      ...
      if (leaderProposal.hash === status.proposalArtifactInfo.hash) {
        ...
        // Check if the facilitator set diverges — this happens when the follower joined
        // a round that the leader started with fewer facilitators (e.g., solo). Validating
        // with a different facilitator set always fails because rewards are computed from
        // the facilitator list. Instead of attempting validation, withdraw immediately.
        ...
        if (facilitatorSetDiverged) {
          ConsensusLog.warn(
            ...
            "reason" -> "facilitator_set_mismatch",
            ...
            Metrics[F].incrementCounter("dag_consensus_facilitator_set_mismatch") >>
            none[Transition].pure[F]
        } else {
