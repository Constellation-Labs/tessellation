...
object GlobalSnapshotEventCutter {
  def make[F[_]: Async: KryoSerializer: Metrics: Clock](
    maxBinarySizeBytes: PosInt,
    snapshotFeeCalculator: SnapshotBinaryFeeCalculator[F]
  ): F[EventCutter[F, StateChannelEvent, DAGEvent]] =
      ...
      new EventCutter[F, StateChannelEvent, DAGEvent] {
        ...
        def cut(
          ...
          Clock[F].monotonic.flatMap { startTime =>
            def logMetrics(result: (List[StateChannelEvent], List[DAGEvent])): F[Unit] =
              Clock[F].monotonic.flatMap { endTime =>
                val duration = endTime - startTime

                (scEvents.toBinary.liftTo[F], dagEvents.toBinary.liftTo[F]).tupled.flatMap {
                  case (scBinary, dagBinary) =>
                    metricsLogger.logEventCutterMetrics(
                      scEvents.size,
                      dagEvents.size,
                      scBinary.length.toLong + dagBinary.length.toLong,
                      duration
                    ) >> scEvents.traverse_ { event =>
                      event.snapshotBinary.toBinary.liftTo[F].flatMap { binary =>
                        metricsLogger.logStateChannelSize(event.address, binary.length.toLong)
                      }
                    }
                }
              }
            ...
            doesNotExceedMaxSize(scEvents, dagEvents)
              .ifM(
                (scEvents, dagEvents).pure[F],
                removeEvents(scEvents, dagEvents, info, ordinal)
              )
              .flatTap(logMetrics)
          }
