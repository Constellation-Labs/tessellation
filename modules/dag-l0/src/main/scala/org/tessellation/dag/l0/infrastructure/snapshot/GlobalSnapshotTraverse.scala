...
trait GlobalSnapshotTraverse[F[_]] {
  def loadChain()(implicit hasher: Hasher[F]): F[(GlobalSnapshotInfo, Signed[GlobalIncrementalSnapshot])]
}
...
object GlobalSnapshotTraverse {
  ...
  def make[F[_]: Async](
    ...
    new GlobalSnapshotTraverse[F] {
      ...
      // TODO: @mwadon Probably BOTH HASHERS
      def loadChain()(implicit hasher: Hasher[F]): F[(GlobalSnapshotInfo, Signed[GlobalIncrementalSnapshot])] = {
        ...
        for {
          ...
          firstInfoCalculatedProof <- hashSelect.select(firstInc.ordinal) match {
          ...
          hashedFirstInc <- firstInc.toHashed
