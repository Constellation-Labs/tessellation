...
object TessellationV3RewardsMigrationSuite extends SimpleIOSuite with Checkers {
  ...
  def createDelegatedRewardsConfig(asOfEpoch: EpochProgress = EpochProgress(100L)): DelegatedRewardsConfig = DelegatedRewardsConfig(
    ...
    emissionConfig = Map(
      AppEnvironment.Dev -> { epochProgress: EpochProgress =>
        Option.when(epochProgress >= asOfEpoch)(
          EmissionConfigEntry(
  ...
  test("v3 reward calculation should use configurable epoch threshold") {
    ...
    IO {
      expect(earlyConfig.emissionConfig(AppEnvironment.Dev)(EpochProgress(10L)).map(_.asOfEpoch) == EpochProgress(10L).some) &&
      expect(lateConfig.emissionConfig(AppEnvironment.Dev)(EpochProgress(5000L)).map(_.asOfEpoch) == EpochProgress(5000L).some)
    }
  }
