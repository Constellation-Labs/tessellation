...
/**
 * Smoke test that the config the dag-l0 node ships actually parses, using the exact `ConfigSource`
 * the app builds at startup. The node loads config in two levels:
 *
 ...
 *
 * both off `TessellationIOApp.loadConfigAs`, i.e. `ConfigSource.resources("dag-l0.conf")` falling
 * back to `ConfigSource.default`. Replicating that here turns a packaged-config regression -- e.g. a
 * refined-type field set to a value its predicate rejects, like the `PosInt` quorum-shrink map that
 * was given `0` (fix e159385fd) -- into a red test instead of a `ConfigReaderException` that crashes
 * every node at startup. See feedback_fix_type_not_revert_feature.
 */
object ConfigLoadSuite extends SimpleIOSuite {
  ...
  test("level 2: AppConfigReader parses, and the per-env quorum-shrink map keeps its 0s (Main startup)") {
    source.loadF[IO, AppConfigReader]().map { cfg =>
      ...
      expect(qs.get(AppEnvironment.Testnet).contains(10)) and
        expect(qs.get(AppEnvironment.Mainnet).contains(0)) and
        expect(qs.get(AppEnvironment.Integrationnet).contains(0)) and
        expect(qs.get(AppEnvironment.Dev).contains(0))
    }
  }
}
