...
object BlockRelationsSuite extends MutableIOSuite with Checkers {
  ...
  test("when no relation between blocks then block is independent") { res =>
    val (hj, hk) = res
    ...
    forall { (block: Signed[Block], notRelatedBlock: Signed[Block]) =>
      for {
        ...
        isRelated = (block: Signed[Block]) => BlockRelations.dependsOn[IO](hashedBlock, txHasher = hk)(block)
        actual <- isRelated(notRelatedBlock)
      } yield expect.same(false, actual)
    }
  }

  test("when first block is parent of second then block is dependent") { res =>
    val (hj, hk) = res
    implicit val hasher: Hasher[IO] = hj
  ...
  test("when second block is parent of first then block is independent") { res =>
    val (hj, hk) = res
    implicit val hasher: Hasher[IO] = hj
  ...
  test("when first block has transaction reference used in second then block is dependent") { res =>
    val (hj, hk) = res
    forall { (block: Signed[Block], notRelatedBlock: Signed[Block]) =>
      for {
        hashedBlock <- {
          implicit val hasher = hj
          block.toHashed[IO]
        }
        isRelated = (block: Signed[Block]) => BlockRelations.dependsOn[IO](hashedBlock, txHasher = hk)(block)
        hashedTxn <- {
          implicit val kryoHasher: SignedHasher[IO] = SignedHasher(hk)
          implicit val proofsHasher: ProofsHasher[IO] = ProofsHasher(hj)
          block.transactions.head.toHashedHybrid[IO]
        }
  ...
  test("when second block has transaction reference used in first then block is independent") { res =>
    val (hj, hk) = res
    forall { (block: Signed[Block], notRelatedBlock: Signed[Block]) =>
      for {
        hashedTxn <- {
          implicit val kryoHasher: SignedHasher[IO] = SignedHasher(hk)
          implicit val proofsHasher: ProofsHasher[IO] = ProofsHasher(hj)
          notRelatedBlock.transactions.head.toHashedHybrid[IO]
        }
  ...
  test("when first block sends transaction to second then block is dependent") { res =>
    val (hj, hk) = res
    implicit val hasher: Hasher[IO] = hj
  ...
  test("when blocks is related with block reference then block is dependent") { res =>
    val (hj, hk) = res
    implicit val hasher: Hasher[IO] = hj
    ...
    forall { (block: Signed[Block], notRelatedBlock: Signed[Block]) =>
      for {
        ...
        isRelated = (block: Signed[Block]) =>
          BlockRelations.dependsOn[IO](Set.empty[Hashed[Block]], Set(hashedBlock.ownReference), txHasher = hk)(block)
        relatedBlock = notRelatedBlock.copy(value =
          notRelatedBlock.value.copy(parent = hashedBlock.ownReference :: notRelatedBlock.value.parent)
        )
        actual <- isRelated(relatedBlock)
