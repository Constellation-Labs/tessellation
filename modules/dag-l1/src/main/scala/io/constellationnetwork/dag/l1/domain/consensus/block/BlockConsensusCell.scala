...
import io.constellationnetwork.dag.l1.domain.consensus.block.BlockConsensusCell._
...
import io.constellationnetwork.dag.l1.domain.consensus.block.CancellationReason._
import io.constellationnetwork.dag.l1.domain.consensus.block.Validator.isReadyForBlockConsensus
import io.constellationnetwork.dag.l1.domain.consensus.block.http.p2p.clients.BlockConsensusClient
import io.constellationnetwork.dag.l1.domain.transaction.{TransactionStorage, transactionLoggerName}
import io.constellationnetwork.effects.GenUUID
import io.constellationnetwork.ext.collection.MapRefUtils.MapRefOps
import io.constellationnetwork.kernel._
import io.constellationnetwork.node.shared.domain.block.processing.BlockValidationParams
...
object BlockConsensusCell {
  ...
  private def pullNewConsensusPeers[F[_]: Async: Random](
    ...
    ctx.clusterStorage.getResponsivePeers
      ...
      .map(_.take(ctx.consensusConfig.peersCount.value).toSet match {
