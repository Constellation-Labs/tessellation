...
object Programs {
  ...
  def make[F[_]: Async: KryoSerializer: SecurityProvider: Random, B <: Block, P <: StateProof, S <: Snapshot[B], SI <: SnapshotInfo[P]](
...
trait Programs[F[_], B <: Block, P <: StateProof, S <: Snapshot[B], SI <: SnapshotInfo[P]] {
  val peerDiscovery: PeerDiscovery[F]
  val l0PeerDiscovery: L0PeerDiscovery[F]
  val joining: Joining[F]
  val snapshotProcessor: SnapshotProcessor[F, B, P, S, SI]
}
