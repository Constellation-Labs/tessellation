...
import org.tessellation.currency.l1.{DummyDataApplicationL1Service, DummyDataApplicationState}
    ...
    forall(gen) {
      case (update, currencyIncrementalSnapshot, maybeEstimateFee) =>
        for {
          ...
          updateHash <- EstimatedFee.getUpdateHash(update, l1Service.serializeUpdate)
          expectedResponse = JsonObject("hash" -> Json.fromValues(List(Json.fromString(updateHash.value))))
      ...
      case (update, currencyIncrementalSnapshot, maybeEstimateFee) =>
        for {
          ...
          expectedResponse = JsonObject(
            "hash" -> Json.fromValues(List(Json.fromString(updateHash.value)))
          )
    ...
    forall(gen) {
      case (update, currencyIncrementalSnapshot, maybeEstimateFee) =>
        for {
          ...
          expectedResponse = JsonObject(
            "feeHash" -> Json.fromValues(
              List(
                Json.fromString(feeTransactionHash.value)
              )
            ),
            "hashes" -> Json.fromValues(
              List(
                Json.fromString(updateHash.value)
              )
            )
          )
