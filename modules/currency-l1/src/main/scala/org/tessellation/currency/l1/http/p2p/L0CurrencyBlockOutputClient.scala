...
trait L0CurrencyBlockOutputClient[F[_]] {
  def sendL1Output(output: Signed[CurrencyBlock]): PeerResponse[F, Boolean]
  def sendL1DataOutput(output: Signed[DataApplicationBlock]): PeerResponse[F, Boolean]
}

object L0CurrencyBlockOutputClient {
  def make[F[_]](l0BlockOutputClient: L0BlockOutputClient[F, CurrencyBlock], client: Client[F])(
