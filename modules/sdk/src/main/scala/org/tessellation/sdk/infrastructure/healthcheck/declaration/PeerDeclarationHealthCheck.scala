...
object PeerDeclarationHealthCheck {
  ...
  ): F[HealthCheckConsensus[F, Key[K], Health, Status[K]]] =
    Ref.of[F, ConsensusRounds[F, Key[K], Health, Status[K]]](ConsensusRounds(List.empty, Map.empty)).map { rounds =>
      val driver = new PeerDeclarationHealthCheckDriver[K]()
      new HealthCheckConsensus[F, Key[K], Health, Status[K]](clusterStorage, selfId, driver, gossip, config) {

        def allRounds: Ref[F, ConsensusRounds[F, Key[K], Health, Status[K]]] = rounds
