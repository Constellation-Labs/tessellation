...
object ConsensusStateUpdater {
  ...
  def make[F[_]: Async: KryoSerializer: SecurityProvider, Event, Key: Show: Order: Next: TypeTag: ClassTag, Artifact <: AnyRef: Show: TypeTag](
    ...
    def tryFacilitateConsensus(
      ...
      consensusStorage
        .condTryModifyState(key)(
          toModifyStateFn(internalTryFacilitateConsensus(key, lastKeyAndArtifact))
        )
        .flatMap(evalEffect)
    ...
    def tryAdvanceConsensus(
      ...
      consensusStorage
        .condTryModifyState(key)(toModifyStateFn(internalTryAdvanceConsensus(key, resources)))
        .flatMap(evalEffect)
        .flatTap(logIfModified(key))
    ...
    private def evalEffect(result: Option[ModifyStateResult]): F[Option[ConsensusState[Key, Artifact]]] =
      result.flatTraverse {
        case (maybeState, effect) => effect >> maybeState.pure[F]
      }
      ...
      maybeState.flatTraverse { state =>
        state.status match {
          ...
          case MajoritySigned(_) =>
            Applicative[F].pure(none[(ConsensusState[Key, Artifact], F[Unit])])
        }
      }
