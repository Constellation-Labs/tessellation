...
object EpochRewards {
  ...
  ): EpochRewards[F] = new EpochRewards[F] {

    def distribute(artifact: Signed[GlobalIncrementalSnapshot], trigger: ConsensusTrigger): F[SortedSet[RewardTransaction]] = {
      val facilitators = artifact.proofs.map(_.id)
      val transactions = artifact.value.blocks.flatMap(_.block.transactions.toSortedSet).map(_.value)
