package io.constellationnetwork.rosetta.domain.construction
...
import io.constellationnetwork.rosetta.domain.amount.{Amount, AmountValue, AmountValuePredicate}
import io.constellationnetwork.rosetta.domain.api.construction.ConstructionMetadata.MetadataResult
import io.constellationnetwork.rosetta.domain.api.construction.ConstructionParse
import io.constellationnetwork.rosetta.domain.api.construction.ConstructionPayloads.PayloadsResult
import io.constellationnetwork.rosetta.domain.error._
import io.constellationnetwork.rosetta.domain.generators._
import io.constellationnetwork.rosetta.domain.operation.OperationType.Transfer
import io.constellationnetwork.rosetta.domain.operation.{Operation, OperationIdentifier, OperationIndex}
import io.constellationnetwork.schema.address.Address
import io.constellationnetwork.schema.generators.{addressGen, transactionSaltGen}
...
object ConstructionServiceSuite extends MutableIOSuite with Checkers with TransactionGenerator {
  ...
  type Res = (SecurityProvider[IO], Hasher[IO], KryoSerializer[IO], Hasher[IO])
  ...
  )(implicit S: SecurityProvider[IO], kryoHasher: SignedHasher[IO], jsonHasher: ProofsHasher[IO]): ConstructionService[IO] =
    ConstructionService.make[IO](getLastRef, salt)

  def generateTestTransactions(wantSignedTransaction: Boolean)(
    implicit S: SecurityProvider[IO],
    kryoHasher: SignedHasher[IO],
    jsonHasher: ProofsHasher[IO]
  ) = {
      ...
      (KeyPairGenerator.makeKeyPair[IO], KeyPairGenerator.makeKeyPair[IO]).tupled.flatMap {
        case (srcKey, dstKey) =>
          ...
          generateTransactions(
            ...
            dstAddress,
            txCount,
            kHasher = kryoHasher.hasher,
            jHasher = jsonHasher.hasher
          )
  ...
  def testParseTransactions(
    ...
    implicit val kryoHasher: SignedHasher[F] = SignedHasher(hasher)
    implicit val proofsHasher: ProofsHasher[F] = ProofsHasher(jh)
  ...
  test("getPayloads returns ParseResult on success") { res =>
    ...
    implicit val hasher = h
    implicit val securityProvider = sp
    implicit val kryoSerializer = ks
    ...
    forall(gen) {
      case (negOp, posOp, metadataResult, salt) =>
        ...
        Hasher.forKryo[IO].hash(dagTransaction).flatMap { txHash =>
