    ...
    (KeyPairGenerator.makeKeyPair[IO], KeyPairGenerator.makeKeyPair[IO]).tupled.flatMap {
      case (srcKey, dstKey) =>
        ...
        for {
          ...
          testValues <- transactions.traverse(hashedTransaction =>
            getBytes(hashedTransaction, wantSignedTransaction)
              .liftTo[IO]
              .map(t => (Hex.fromBytes(t), hashedTransaction.hash, hashedTransaction))
          )
        } yield testValues
    }
