...
const postNodeParamsNodeId = async (
  ...
  try {
    ...
    console.info(
      `Failed to fetch parent reference for node ${nodeId}, using default parent`,
    )
  }
...
const createTokenLock = async (account, urls, lockAmount) => {
  ...
  const { hash } = await dag4.account.postTokenLock({
  ...
  await withRetry(
    async () =>
      assertBalanceChange(dag4.account, initialBalance - lockAmount / 1e8),
    {
...
const withdrawDelegatedStake = async (account, stakeHash) => {
  const { hash } = await dag4.account.putWithdrawDelegatedStake({
    source: account.address,
    stakeRef: stakeHash,
  })
...
const checkWithdrawDelegatedStake = async (urls, account, stakeHash) => {
  ...
  const lockPlusRewardsDag = originalStake.totalBalance / 1e8

  await assertBalanceChange(account, initialBalance + lockPlusRewardsDag)
