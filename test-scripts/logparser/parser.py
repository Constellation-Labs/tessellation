...
def main():
    ...
    # Print the top 100 IPs with their occurrence count
    print("\nTop 100 IPs by occurrence count:")
    print("-" * 80)
    print(f"{'IP Address':<20} {'Count':<10} {'In JSON List':<15}")
    print("-" * 80)
    
    for ip, count in sorted_ips:
        in_json = "Yes" if ip in known_ips else "No"
        print(f"{ip:<20} {count:<10} {in_json:<15}")
