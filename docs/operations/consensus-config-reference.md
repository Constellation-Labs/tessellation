...
| `certified-consensus-activation-ordinal` | `Map[Env, SnapshotOrdinal]` | DAG IntegrationNet `5890500`; dev `0`; all Currency public entries absent | **disabled** (`SnapshotOrdinal.MaxValue`) | Per-L0 v35 behavior boundary. DAG L0 and every Currency L0 interpret this in their own ordinal space. The resolved key is hash-folded and must be installed cluster-wide before the announced activation. |
...
uses node-local actual-proof history and reuses the compiled-in
