...
| `tightening-window` | `CL_TIGHTENING_WINDOW` | Int | `10` | Size (in ordinals) of the rolling `recentSigners` window that feeds tier-demotion hysteresis and the reward-rotation evidence window. LIVE (`config/types.scala:296-315`). |
...
| `active-facilitator-target` | `CL_ACTIVE_FACILITATOR_TARGET` | Option[Int] | `None` (conf: `7`) | Deterministic active-facilitator expansion target. A target, not a cap: more recent signers stay active up to `active-facilitator-max` (`config/types.scala:318-323`). |
| `active-facilitator-max` | `CL_ACTIVE_FACILITATOR_MAX` | Option[Int] | `None` (conf: `13`) | Hard cap on deterministic active-facilitator admission. `None` keeps the legacy selected-pool size (`config/types.scala:324-327`). |
...
| `reward-rotation-epoch-rounds` | `Map[Env, Int]` (override `CL_REWARD_ROTATION_EPOCH_ROUNDS`) | testnet `3` (`dag-l0.conf:241-244`) | **disabled** (resolved `0`, byte-identical to pre-rotation) | `RewardRotation` epoch length in ordinals over which exactly ONE Tier-1 (non-quorum) signing seat is deterministically rotated among demonstrated-live peers, so reward share is not pinned to the always-on signer set. Core is never rotated (`config/types.scala:873-884, 1194-1202`). |
| `reward-rotation-eligibility-window` | `Map[Env, Int]` (override `CL_REWARD_ROTATION_ELIGIBILITY_WINDOW`) | testnet `10` (`dag-l0.conf:253-256`) | full evidence window (resolved `0` -> `tightening-window`) | Trailing evidence-window length over which a peer must have signed at least once to be eligible to rotate IN. MUST satisfy `epoch < eligibilityWindow <= tighteningWindow` or a rotated-out peer is benched forever (`config/types.scala:885-898, 1203-1212`). |

> **Fairness invariant for reward rotation** (`dag-l0.conf:230-240`): `(waiting-pool-size * reward-rotation-epoch-rounds) <= reward-rotation-eligibility-window <= tightening-window`. Larger waiting pools need a larger `tightening-window`.
...
2. Per-environment knobs (`core-committee-size`, `quorum-shrink-activation-views`, `reward-rotation-*`, `active-admission-min-probation-reentry-slots`, `active-admission-recent-signer-window`) must match cluster-wide; **adding or removing an env entry is a consensus change** because absent means a specific resolved default.
