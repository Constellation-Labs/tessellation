...
| `active-admission-min-probation-reentry-slots` | `Map[Env, Int]` | testnet `3`, mainnet `15`, integrationnet `9`, dev `3` | disabled (resolved `0`) | Minimum bounded probation cohort admitted even when the per-round expansion budget is exhausted. A peer that signs the latest round retains priority for a non-Core seat until it reaches the retain band; missing ends that lease. Current public GL0 environments configure one Core-sized cohort. |
