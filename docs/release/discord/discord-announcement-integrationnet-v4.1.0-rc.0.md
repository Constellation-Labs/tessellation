# Discord Announcement - IntegrationNet v4.1.0-rc.0
...
**Hard fork:** Yes - coordinated cold restart, all nodes on the same release artifact and config
...
> **This is a coordinated cold restart, not a rolling upgrade.** All nodes must run the same release
> artifact and consensus config. A node on a mismatched version or config is refused at the connection
> handshake.
...
> **Metagraph operators must update their Tessellation dependency to v4.1.0-rc.0 and rebuild before
...
## Second post: Network Update (published on 2026-07-13 at restart)
...
- **Hard fork:** Yes. Every node is on the v4.1.0-rc.0 release with matching consensus configuration.
