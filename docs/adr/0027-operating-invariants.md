...
1. **The release-version hash and L0 consensus-config hash are independent connect-time fences; `FieldsAddedOrdinals` gates replay.** Joining checks the Tessellation version hash, metagraph version hash, and environment. L0 also advertises the hash of its exact effective consensus config and requires strict equality, including presence; Facility comparison remains a second diagnostic. Operators must verify both fingerprints fleet-wide. `RegistrationRequest.jar` is advertised and stored but is not compared during joining, so operators must also verify identical release artifacts outside the protocol. None of these values gates replayed history -- that is the job of the ordinal gates (`FieldsAddedOrdinals`, ADR-0025). A wrong ordinal is NOT caught at handshake; it forks the chain when the gate is crossed.
