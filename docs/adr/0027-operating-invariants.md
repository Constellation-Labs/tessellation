...
1. **The release-version hash is the connect-time fence; the consensus-config hash remains a required compatibility fingerprint; `FieldsAddedOrdinals` gates replay.** Joining checks the Tessellation version hash, metagraph version hash, and environment. It can compare `consensusConfigHash` when both peers provide it, but current shared startup leaves that optional registration field unset; Facility mismatches are currently diagnostic rather than rejecting. Operators must therefore verify `deterministicConfigHash` equality fleet-wide even though `versionHash` is the hard rc-to-rc join fence. `RegistrationRequest.jar` is advertised and stored but is not compared during joining, so operators must also verify identical release artifacts outside the protocol. None of these values gates replayed history -- that is the job of the ordinal gates (`FieldsAddedOrdinals`, ADR-0025). A wrong ordinal is NOT caught at handshake; it forks the chain when the gate is crossed.
