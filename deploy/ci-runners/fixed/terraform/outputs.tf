...
output "concurrency" {
  description = "Concurrent E2E jobs this pool can serve (one runner per server), and the resulting wave count for the 11-job matrix."
  value       = "${var.runner_count} concurrent; ${ceil(11 / var.runner_count)} wave(s) for the 11-job E2E matrix"
}
