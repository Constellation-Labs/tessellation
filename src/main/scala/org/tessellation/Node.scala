...
case class Node(id: String, txPool: L1TransactionPoolEnqueue) {
  ...
  def startL1Consensus(edge: L1Edge[L1Transaction]): IO[Unit] = for {
    peers <- peers.get
...
object Node {
  ...
  private def generateRandomTxPool: IO[L1TransactionPoolEnqueue] =
    List(1, 2, 3).traverse(_ => IO.delay {
      Random.nextInt(Integer.MAX_VALUE)
    }.map(L1Transaction)).map(_.toSet) >>= L1TransactionPool.init
}
