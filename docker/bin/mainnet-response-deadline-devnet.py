#!/usr/bin/env python3
"""Five-validator gossip response starvation A/B; isolated lab only."""
import argparse
...
from mainnet_body_fault import BodyFaultController
...
def recovery_agreed(tips, baseline, rounds):
    healthy = tips[:3]
    return (len(healthy) == 3 and all(t and t["ordinal"] >= baseline + rounds for t in healthy)
            and len({(t["ordinal"], t["digest"]) for t in healthy}) == 1)
...
def main():
    ...
    if args.mode == "stock" and expected != "9a5726027b962f3a8271d77c37e9c11c66d10a5a960da44d06c283b2a523ed5d":
        parser.error("stock requires official v3.5.30 JAR")
    ...
    prefix = "mr-body-" + args.mode + "-" + str(int(time.time()))
    network = prefix
    ...
    try:
        event("identity", mode=args.mode, peer_gossip_workers=2, mainnet_default_workers=8, jar_sha256=hashlib.sha256(jar.read_bytes()).hexdigest(),
        ...
        def start(i):
            ...
            cmd = ["docker", "run", "-d", "--name", names[i], "--network", network, "--ip", f"172.30.194.{10+i}",
                   ...
                   "-Dgossip.daemon.peer-round.max-concurrent-rounds=2",
        ...
        phase_controller = BodyFaultController(run, names, network, peers, event,
                                               row["tips"][0]["ordinal"] + args.healthy_rounds, out, args.mode)
        ...
        while time.monotonic() - start_measure < args.seconds:
            ...
            if phase_controller:
                ...
                if (phase_controller.finished.is_set() and recovery_baseline is None
                        and all(row["tips"][:3])):
                    # Anchor AFTER restoration, not at the affected ordinal: the
                    # candidate may already advance while the fault is active.
                    recovery_baseline = max(t["ordinal"] for t in row["tips"][:3])
                    event("post_restoration_baseline", ordinal=recovery_baseline)
                if (recovery_baseline is not None
                        and recovery_agreed(row["tips"], recovery_baseline, args.post_fault_rounds)):
                    event("post_fault_rounds_observed", count=args.post_fault_rounds,
                          fault_ordinal=phase_controller.fault_ordinal,
                          restoration_baseline=recovery_baseline, tips=row["tips"])
                    break
            if int(elapsed) % 30 < 5:
