...
check_health() {
  ...
  while [ $retry_count -lt $MAX_RETRIES ]; do
      ...
      if [ "$CLUSTER_INFO_LEN" = "$num_expected_nodes" ]; then
        echo "Success: cluster $service $i has $num_expected_nodes nodes on port $port"
        break
      else
        echo "Waiting for $service node $i on port $port to have $num_expected_nodes nodes, currently $CLUSTER_INFO_LEN nodes retry $retry_count of $MAX_RETRIES"
        sleep 4
        retry_count=$((retry_count+1))
      fi
      
      if [ $retry_count -gt $((MAX_RETRIES-2)) ]; then
        echo "ERROR: $service cluster $i doesn't have $num_expected_nodes nodes on port $port after $MAX_RETRIES attempts"
        return 1
      fi
    done
}
...
verify_healthy() {
  ...
  for i in "0" "1" "2"; do
      ...
      if [ "$i" -lt "$NUM_GL0_NODES" ]; then
        check_health $l0_port $i "gl0" $NUM_GL0_NODES
      fi

      if [ "$i" -lt "$NUM_GL1_NODES" ]; then
        check_health $l1_port $i "gl1" $NUM_GL1_NODES
      fi

      if [ "$i" -lt "$NUM_ML0_NODES" ]; then
        check_health $ml0_port $i "ml0" $NUM_ML0_NODES
      fi

      if [ "$i" -lt "$NUM_CL1_NODES" ]; then
        check_health $cl1_port $i "cl1" $NUM_CL1_NODES
      fi
      
      if [ "$i" -lt "$NUM_DL1_NODES" ]; then
        check_health $dl1_port $i "dl1" $NUM_DL1_NODES
      fi
